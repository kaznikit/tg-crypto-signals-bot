...
| `ENTRY_DELAY_CANDLES` | подождать N закрытых свечей `STOP_TIMEFRAME` после сигнала перед входом (0 — входить сразу) |
...
2. Если задан `ENTRY_DELAY_CANDLES > 0` — бот ждёт указанное число закрытых свечей `STOP_TIMEFRAME` (например, при `ENTRY_DELAY_CANDLES=3` и `STOP_TIMEFRAME=5` — это 15 минут), присылая уведомление о начале ожидания. Это осмысленно, если после сигнала часто сначала идёт откат, а не сразу движение в сторону сигнала: пауза перед входом даёт цене время на откат, а весь дальнейший расчёт (шаги 3-6) выполняется уже по актуальным на момент входа данным, а не по цене в момент самого сигнала.
3. Если по символу уже есть открытая позиция — сигнал пропускается (уведомление в канал сделок).
4. Загружаются последние закрытые свечи выбранного таймфрейма (5m/15m) и ищется ближайший (самый свежий) свинг-хай (для шорта) или свинг-лоу (для лонга) — точка, которая выше/ниже `SWING_WINDOW` соседних свечей с каждой стороны.
5. Стоп = экстремум + отступ `STOP_OFFSET_PCT`% (за хай для шорта, за лоу для лонга).
6. Вход — рыночный ордер, объём считается из `ORDER_SIZE_USDT * LEVERAGE / цена`, округляется вниз до шага лота инструмента.
7. Тейк = вход ± (риск на юнит × `RISK_REWARD`), стоп и тейк выставляются вместе с ордером через Bybit API (`stopLoss`/`takeProfit`).
8. Обо всех действиях (жду N свечей / открыл сделку / пропустил / ошибка) бот пишет в настроенный канал уведомлений через Bot API.

Технический момент: ожидание не блокирует обработку других сигналов — Telethon по умолчанию обрабатывает каждое входящее сообщение в отдельной asyncio-задаче (`sequential_updates=False`), так что параллельно можно "ждать" сразу по нескольким сигналам на разных активах.
